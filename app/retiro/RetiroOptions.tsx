...
import { Bitcoin, CreditCard, Play, CircleDollarSign, Check } from "lucide-react";
...
export const RetiroOptions = ({ balance }: RetiroOptionsProps) => {
  ...
  const balanceInUsd = balance / 1000;
  ...
  const renderCard = (option: any, delayIdx: number) => {
    ...
    return (
      <motion.div
        ...
        onClick={() => {
          if (isEligible) {
            alert(`Solicitud de retiro de $${selectedAmount} USD a ${option.name} enviada.`);
          }
        }}
      >
