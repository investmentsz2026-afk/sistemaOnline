...
import { User, Coins, Play, Trophy, Users, CheckCircle2, AlertCircle, Sparkles, Plus, Swords, LogIn, XCircle } from "lucide-react";
...
const SEGMENT_COLORS = ["#FF6B6B", "#FFD93D", "#6BCB77", "#4D96FF", "#937DC2", "#FF8B13"];
...
export const RouletteArena = ({ initialBalance }: RouletteArenaProps) => {
  ...
  const handleStartSpin = () => {
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        const winIdx = Math.floor(((360 - (finalRotation % 360) + 30) % 360) / 60);
        setWinner(winIdx);
        ...
        setTimeout(() => {
          ...
          if (joinedPlayers[winIdx]?.isUser) {
            setBalance(prev => prev + (currentRoom!.priceCoins * 6));
          }
        }, 1000);
  ...
  return (
    <div className="space-y-12 pb-32">
      {/* MENSAJES GLOBALES (Fuera de cualquier contenedor para ocupar TODA la pantalla) */}
      <AnimatePresence>
        {errorModal?.show && (
        ...
        {showWinnerModal && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[300] bg-[#050a1f]/95 backdrop-blur-3xl flex flex-col items-center justify-center p-6 text-center">
            <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="space-y-6 max-w-full">
              ...
              <div className="text-xl md:text-3xl font-black text-emerald-400">GANÓ ${(currentRoom?.priceUsd ? currentRoom.priceUsd * 6 : 0).toFixed(2)} USD</div>
            </motion.div>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      {currentRoom && (
        <div className="bg-[#0b0e14]/60 border border-white/5 rounded-[4rem] p-6 md:p-16 backdrop-blur-3xl relative shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-500">
          <div className="flex flex-col lg:flex-row items-center gap-16 md:gap-24">
            <div className="relative w-full max-w-[340px] md:max-w-[480px] aspect-square">
              ...
              <motion.div 
                ...
                style={{ 
                  background: `conic-gradient(from 330deg, ${SEGMENT_COLORS[0]} 0deg 60deg, ${SEGMENT_COLORS[1]} 60deg 120deg, ${SEGMENT_COLORS[2]} 120deg 180deg, ${SEGMENT_COLORS[3]} 180deg 240deg, ${SEGMENT_COLORS[4]} 240deg 300deg, ${SEGMENT_COLORS[5]} 300deg 360deg)` 
                }}
              >
                {[...Array(6)].map((_, i) => (
                  <div 
                    ...
                    style={{ transform: `translateX(-50%) rotate(${i * 60}deg)`, transformOrigin: "50% 143%" }}
                  >
                    <span className="text-slate-900 font-black text-[12px] md:text-[20px] uppercase tracking-tighter [writing-mode:vertical-rl] rotate-180 drop-shadow-sm max-h-[85%] overflow-hidden text-center leading-none">
                      {joinedPlayers[i] ? `${joinedPlayers[i].name} #${joinedPlayers[i].id}` : "VACÍO"}
                    </span>
                  </div>
                ))}
                {[...Array(6)].map((_, i) => (
                  <div key={`pin-${i}`} className="absolute top-0 left-1/2 w-1.5 h-6 bg-white/20 shadow-sm" style={{ transform: `translateX(-50%) rotate(${i * 60 + 30}deg)`, transformOrigin: "50% 833%", borderRadius: "0 0 10px 10px" }} />
                ))}
              </motion.div>
            ...
            <div className="flex-1 w-full space-y-8">
              ...
              <div><div className="text-[10px] font-black text-slate-500 uppercase tracking-[0.5em] mb-2">Jackpot Acumulado</div><div className="text-6xl md:text-8xl font-black text-white tracking-tighter italic leading-none">${(currentRoom.priceUsd * 6).toFixed(2)}</div></div>
